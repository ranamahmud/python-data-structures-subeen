...
def test_append():
    ...
    assert str(dll) == "5,8", "String representation of dll shoudl match 5,8"
def test_prepend():
    dll = DoublyLinkedList()
    ...
    assert new_node.prev == None, "previous nodeof new_node should be None"
    assert old_node.prev == new_node, "prevoius node of the old node should be new_node"
...
def test_search():
    ...
    item = 5
    assert dll.search(item) == None, "item shouln't be found in an empty list"
    dll.append(item)
